...
interface IDataTableProps {
  ...
  showHideModal: (id: string) => void;
  selectedItemId: string;
  showModal: boolean;
}

const DataTable: React.FunctionComponent<IDataTableProps> = (props) => {
  const { data, updateReloadState, showHideModal, showModal, selectedItemId } =
    props;
    ...
    return data.map((item) => {
      return (
        ...
        <TableRow
          key={item._id}
          item={item}
          onCopy={() => copyToClipboard(item.shortUrl)}
          reload={() => updateReloadState()}
          onDelete={() => {
            // setSelectedItemId(item._id);
            // setShowModal(true);
            showHideModal(item._id);
          }}
        />
      );
    });
